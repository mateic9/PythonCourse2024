...
def get_move_from_player(client_socket,game):
    ...
    client_socket.sendall(f"Valid move:{move}".encode('utf-8'))
    return move
def handle_client_1p(client_socket, client_address,game_finished,game):
    ...
    while not game_finished.is_set():
        if move_index % 2 == 0:
            ...
            current_move=int(AI_decides_move())
...
def handle_client_2p(client_socket, client_address, client_id,players_barrier,e1,e2,game_finished,game):
    ...
    try:
        ...
        client_socket.sendall(f"End of Game!Player with id:{win_id} won".encode('utf-8'))
...
def start_server(host='127.0.0.1', port=65432):
    ...
    while client_id <= 2:
        ...
        if client_id==1:
            game=LogicGame(int(width),int(height))
        ...
        if mode=='multiplayer':
            ...
            if client_id==1:
              thread = threading.Thread(target=handle_client_2p, args=(client_socket, client_address, client_id,wait_2players_barrier,e1,e2,game_finished,game))
              threads.append(thread)
              thread.start()
            else:
              if game.height!= int(height) or game.width!=int(width):
                ...
                client_socket.close()
                  ...
                  thread = threading.Thread(target=handle_client_2p, args=(client_socket, client_address, client_id,wait_2players_barrier,e2,e1,game_finished,game))
                  threads.append(thread)
                  thread.start()
        client_id += 1
